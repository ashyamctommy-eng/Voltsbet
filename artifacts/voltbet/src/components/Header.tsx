...
export default function Header({
  ...
  const router = { push: navigate, refresh: () => window.location.reload() };
  const { push } = useToast();
...
function MobileSearch() {
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        if (q.trim().length > 0) navigate(`/search?q=${encodeURIComponent(q.trim())}`);
      }}
    >
