...
const GROUPS: { title: string; anchor: string; icon: React.ReactNode; fields: Field[] }[] = [
  ...
  },
  {
    ...
    icon: <IconGear className="h-4 w-4" />,
    fields: [
      { key: "odds.provider", label: "Odds provider", type: "select", options: ["betsapi", "the-odds-api", "api-football", "odds-api-io"], hint: "betsapi = Bet365 via RapidAPI (PRIMARY, creds in Admin → API Settings) · the-odds-api = The Odds API (ODDS_API_KEY) · api-football = API-Football (ODDS_API_IO_KEY) · odds-api-io = Odds-API.io (ODDS_IO_KEY)" },
