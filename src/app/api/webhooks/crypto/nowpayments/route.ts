...
export async function POST(req: NextRequest) {
  ...
  switch (status) {
    case "finished":
      // Map NOWPayments IPN fields onto our deposit record: pay_address is
      // the receiving deposit address (stored on metadata.depositAddress),
      // hash is the on-chain transaction hash (stored on Deposit.txHash).
      await confirmDeposit(deposit.id, {
        txHash: String(body.hash ?? ""),
        depositAddress: body.pay_address,
