  ...
  return (
    <div className="mx-auto max-w-[1600px] px-4">
      <div className="mt-4 flex items-center justify-between">
        ...
        <span className="rounded-full bg-brand/15 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-brand">Coming soon</span>
      </div>
      <p className="mt-1 text-sm text-ink2">
        Crash games and slots are being wired up — these tiles launch the moment the first provider goes live.
      </p>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {GAMES.map(({ name, Icon, bg, desc, tag }) => (
          <div key={name} className={`card card-hover relative overflow-hidden bg-gradient-to-br ${bg} p-5`}>
            {tag && (
              <span className="absolute right-3 top-3 rounded-full bg-red-500 px-2 py-0.5 text-[10px] font-black text-white">{tag}</span>
            )}
            ...
            <button disabled className="btn btn-primary btn-sm mt-4 !cursor-not-allowed !opacity-60">Launch — soon</button>
          </div>
        ))}
      </div>
