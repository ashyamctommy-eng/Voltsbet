/**
 * The Odds API sport key → display league name ("soccer_epl" →
 * "England - Premier League"). Stamped onto games by the sync and used by
 * the feed + match-view for display. Dependency-free leaf module: match-view
 * and feed both import it, so no cycle can form between them (SSR chunk
 * TDZ safety — "Cannot access X before initialization").
 */
