# Deploying Voltbets to Railway (testing) — step by step
...
1. Go to **railway.app** → **New Project** → **Deploy from GitHub repo** → pick `voltbets`.
...
1. **Variables** tab → add:
   ...
   | `SEED_ADMIN_EMAIL` | *(recommended)* super-admin email — defaults to `admin@voltbets.test` |
...
2. Deploy. When it finishes, open the service URL → you should see Voltbets.
...
- Local dev only: seeded demo accounts are `demo@voltbets.test` / `Demo123!` (see README).
...
2. **Voltbets → Admin → Website Settings → Crypto Payments:**
...
1. **developer.safaricom.co.ke** → register → create an **app** → get Consumer Key
   ...
   certificate** for B2C.
2. **Voltbets → Admin → Website Settings → M-Pesa:**
   - `mpesa.enabled` = `true`
   - `mpesa.env` = `sandbox`
...
```bash
# on your VPS / hosting
git clone https://github.com/YOU/voltbets.git /opt/voltbets
cd /opt/voltbets
pnpm install
